...
sjcl.bn.prototype = {
    ...
    montpowermod: function (a, b) {
        ...
        function c(a, d) {
            ...
            for (c = 0; c < e.limbs.length; c++) 0 < c && (e.limbs[c - 1] |= (e.limbs[c] & f) <<
                g - l - 1), e.limbs[c] >>= l + 1;
        ...
        for (var r = new sjcl.bn(0),
                t = k.copy(); t.greaterEquals(1);) t.halveM(), 0 == (f.limbs[0] & 1) ? (f.halveM(), r.halveM()) : (f.addM(p), f.halveM(), r.halveM(), r.addM(n));
        f = f.normalize();
        r = r.normalize();
        n.doubleM();
        p = n.mulmod(n, b);
...
sjcl.bn.pseudoMersennePrime = function (a, b) {
    ...
    for (e = 0; e < b.length; e++) c.offset[e] = Math.floor(b[e][0] / c.radix - f), c.fullOffset[e] = Math.floor(b[e][0] / c.radix) - g + 1, c.factor[e] = b[e][1] * Math.pow(.5, a - b[e][0] + c.offset[e] * c.radix),
        c.fullFactor[e] = b[e][1] * Math.pow(.5, a - b[e][0] + c.fullOffset[e] * c.radix), c.modulus.addM(new sjcl.bn(Math.pow(2, b[e][0]) * b[e][1])), c.minOffset = Math.min(c.minOffset, -c.offset[e]);
    c.o = d;
...
sjcl.arrayBuffer.ccm = {
    ...
    compat_decrypt: function (a, b, d, c, e) {
        ...
        var f = sjcl.bitArray,
            ...
            h = f.clamp(b, g - e);
        b = f.bitSlice(b, g - e);
        h = sjcl.codec.arrayBuffer.fromBits(h,
            !0, 16);
